  ...
  <div class="space-y-4 animate-fade-in">
    ...
    <div class="space-y-3">
      ...
      <div class="relative w-full">
        <svg class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-dark-500 pointer-events-none" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div class="grid grid-cols-2 gap-2">
        ...
        <select v-model="filterAssignee" class="w-full text-sm bg-dark-800 border border-dark-700 rounded-xl px-3 py-2 text-dark-300 focus:outline-none focus:border-accent-500">
          <option value="">Membro</option>
          <option v-for="m in currentProject?.members ?? []" :key="m.id" :value="m.id">{{ m.name.split(' ')[0] }}</option>
        </select>
      </div>

      <!-- Filtros rápidos de urgência + limpar -->
      <div class="flex flex-wrap items-center gap-1.5">
        ...
        <button
          ...
          class="text-xs text-dark-500 hover:text-dark-300 px-2 py-1.5 rounded-lg hover:bg-dark-700 transition-colors ml-auto"
        >Limpar</button>
      </div>
    </div>
...
const quickFilters = [
  ...
  { value: 'week',    label: 'Essa semana', activeClass: 'bg-green-500/15 border-green-500/30 text-green-400' },
]
